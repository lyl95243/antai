...
$(function () {
    ...
    // 表格数据
    Ajax(
        "http://172.16.10.121:8080/smarteye/api/search/security/relativeLogs?eventTime=" + attkTime + "&attkIP=" + attkIP + "&servIP=" + servIP + "&proto=&type=0&interval=3&page=1&pageSize=12",
        "get",
        "json",
        "",
        false,
        function (result) {
            ...
            $.each(arrResult, function (i) {
                ...
                var tableCon = '<td>' + arrResult[i].accessTime + '</td>' +
                    '<td>' + arrResult[i].servIP + '</td>' +
                    '<td></td><td>'+isAttk+'</td>' +
                    '<td><a href="JavaScript:" class="trace">查看详情</a></td>'
            ...
            var options = {
                ...
                itemTexts: function (type, page, current) {
                    ...
                    Ajax(
                        "http://172.16.10.121:8080/smarteye/api/search/security/relativeLogs?eventTime=" + attkTime + "&attkIP=" + attkIP + "&servIP=" + servIP + "&proto=&type=0&interval=3&page="+page+"&pageSize=12",
                        "get",
                        "json",
                        "",
                        false,
                        function (result) {
                            ...
                            var tb = $("#tablee");
                            tb.html("");
                            var tableTh = '<th>访问时间</th><th>服务器IP</th><th>利用协议</th><th>是否攻击</th><th>深度分析</th>'
    ...
    $(".chaxun").click(function () {
        Ajax(
            "http://172.16.10.121:8080/smarteye/api/search/security/relativeLogs?eventTime=" + attkTime + "&attkIP=" + attkIP + "&servIP=" + servIP + "&proto=&type="+attkAB+"&interval="+inputValue+"&page=1&pageSize=12",
            "get",
            "json",
            "",
            false,
            function (result) {
                ...
                var options = {
                    ...
                    itemTexts: function (type, page, current) {
                        ...
                        Ajax(
                            "http://172.16.10.121:8080/smarteye/api/search/security/relativeLogs?eventTime=" + attkTime + "&attkIP=" + attkIP + "&servIP=" + servIP + "&proto=&type="+attkAB+"&interval="+inputValue+"&page="+page+"&pageSize=12",
                            "get",
                            "json",
                            "",
                            false,
